...
public class Controller {
  ...
  /**
   * Controller constructor that takes in and Random as well
   *
   * @param view the view of the game
   * @param input the input that is being read from
   * @param rand the Random being passed in
   */
  public Controller(ViewImpl view, Readable input, Randomable rand) {
    this.view = view;
    this.input = input;
    this.scan = new Scanner(input);
    this.rand = rand;
  ...
  public void prepareGame() throws IOException {
    ...
    while (inValidFleet) {
      ...
      if (numOfCarrier + numOfBs + numOfDes + numOfSm > maxSize) {
        ...
        player1 = new PlayerImpl(height, width, view, rand);
  ...
  public void gameInPlay() throws IOException {
    ...
    while (gameNotOver) {
      ...
      GameResult.Result result = gameResult
          .returnResult(player1.returnShips(), aiPlayer.returnShips());
      if (result.equals(GameResult.Result.INPLAY)) {
        continue;
      } else {
        player1.endGame(gameResult, "Some reason");
        aiPlayer.endGame(gameResult, "some reason");
        gameNotOver = false;
